...
-(void) captureOutput:(AVCaptureOutput *)output didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    @autoreleasepool {
        ...
        CVPixelBufferRef pixelBuffer = scaledPixelBufferMDS(ref, {_width, _height});
...
@implementation GrapeEmulator
+(GrapeEmulator *) sharedInstance {
    static GrapeEmulator *sharedInstance = NULL;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[self alloc] init];
    });
...
-(void) insertCartridge:(NSURL *)url {
    ...
    FILE* sav = Platform::OpenFile(savename, "rb", true);
    if (!sav) sav = Platform::OpenFile(origsave, "rb", true);
    if (sav)
    {
